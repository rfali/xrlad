...
from xrl import shap_explain
from plot import plot_patient
...
def main(args):
    ...
    name = name.replace("_frontal_hippo_atleast3","")  # Modifying name string
    ...
    log_dir = f'progress/{name}_{args.algo}_seed{seed}'
    ...
    def train_policy(ctxt=None, seed=1, n_epochs=50, batch_size=1000, action_type='delta', 
        ...
        with LocalTFRunner(snapshot_config=ctxt) as trainer:
            ...
            # Define the policy for the environment
            ...
            if algo_name == 'TRPO':
                ...
                # Create a VPG (Vanilla Policy Gradient) algorithm instance.
                algo = VPG(
                    env_spec=env.spec,
                    policy=policy,
                    baseline=baseline,
    ...
    state_log_train, action_log_train, output_train = evaluator.simulate(train_data, 'train', scale_state, args.normalize)
    ...
    if args.shap_enable:
        print('Plotting SHAP...')   
        shap_explain(log_dir, output_dir, state_log_train, action_log_train, split_type='train', 
                    use_all_samples=args.shap_use_all_samples, show_fig=args.shap_show_fig)

    # Evaluate on the Validation set
    print('\n*****Starting evaluation on Validation set*****')
    # Simulate the policy on the validation data. Log states and actions
    state_log_valid, action_log_valid, output_valid  = evaluator.simulate(valid_data, 'valid', scale_state, args.normalize)
    ...
    if args.shap_enable:   
        print('Plotting SHAP...')   
        shap_explain(log_dir, output_dir, state_log_valid, action_log_valid, split_type='valid', 
                    use_all_samples=args.shap_use_all_samples, show_fig=args.shap_show_fig)

    # Evaluate on the Test set
    print('\n*****Starting evaluation on Test set*****')
    # Simulate the policy on the test data. Log states and actions
    state_log_test, action_log_test, output_test = evaluator.simulate(test_data, 'test', scale_state, args.normalize)
    ...
    # Generate SHAP plots for the test set
    if args.shap_enable:
        print('Plotting SHAP...')   
        shap_explain(log_dir, output_dir, state_log_test, action_log_test, split_type='test', 
                    use_all_samples=args.shap_use_all_samples, show_fig=args.shap_show_fig)

    # Evaluate patient data for certain years
    # if args.shap_enable:
    #     print('\n*****Plotting SHAP for specific years [0,3,6,9]*****')
    #     years = np.unique(train_df['Years'].values) 
    #     selected_years = np.array([0, 3, 6, 9]) 
    #     for y in selected_years:
    #         train_df_year = train_df.loc[(train_df['Years'] == y)]
    #         year_data = extract_data(args, train_df_year, H, gamma)
    #         state_log_year, action_log_year, _ = evaluator.simulate(year_data, f'year_{y}', scale_state, args.normalize)
    #         state_log_year = np.nan_to_num(state_log_year, nan=0.0)
    #         state_log_year = np.where(np.isinf(state_log_year), 999.0, state_log_year)
    #         shap_explain(log_dir, output_dir, state_log_year, action_log_year, split_type=f'per_year/year_{y}', 
    #                 use_all_samples=args.shap_use_all_samples, show_fig=args.shap_show_fig)        

    # Evaluate patient data for patients who had the highest decline in cognition scores.
    if args.shap_enable:
        print('\n*****Plotting SHAP for specific patients based on selected features*****')
        rids = np.unique(test_df['RID'].values)
        df_join = pd.merge(test_df, output_test, how='right', left_on=['RID', 'Years'], right_on=['RID', 'Years'])
        
        # Does a per-patient evaluation
        # filter_feature: patients are filtered by checking which have the most decline in this feature
        # num_patients: the number of patients to analyze after filtering
        def per_patient_evaluation(filter_feature: str, num_patients: int):
            # Get the num_patients patients with at least 5 years of info who have the largest difference in the specified feature
            rid_dict = dict()
            for rid in rids:
                df_patient = df_join.loc[df_join['RID'] == rid, filter_feature].dropna()
                if len(df_patient) > 5:
                    baseline_year = df_patient.iloc[0].item()
                    last_year = df_patient.iloc[len(df_patient) - 1].item()
                    rid_dict[rid] = baseline_year - last_year
            sorted_rids = [i[0] for i in sorted(rid_dict.items(), key=lambda x: x[1], reverse=True)[:num_patients]]

            # Evaluate the selected patients
            for rid in sorted_rids:
                print(f'\nEvaluating RID {rid}...')
                df_new = test_df.loc[test_df['RID'] == rid]   
                patient_data = extract_data(args, df_new, H, gamma)
                state_log_patient, action_log_patient, df_output = evaluator.simulate(patient_data, f'RID_{rid}_{filter_feature}', scale_state, args.normalize)

                # Plot the trajectory plots (df_new = ground truth, df_output = rl prediction)
                df_plot = pd.merge(df_new, df_output.loc[df_output['RID'] == rid], how='right', left_on=['RID', 'Years'], right_on=['RID', 'Years'])
                print('Plotting per-patient trajectory plots...')
                plot_dir = f'{output_dir}/plots/per-patient/'
                os.makedirs(plot_dir, exist_ok=True)
                plot_patient(df_plot, f'{plot_dir}RID-{rid}_{filter_feature}', score=args.score)

                print('Plotting SHAP...')   
                shap_explain(log_dir, output_dir, state_log_patient, action_log_patient, split_type=f'per_patient/RID_{rid}_{filter_feature}', 
                        use_all_samples=args.shap_use_all_samples, show_fig=args.shap_show_fig)
                

        # call the function for MMSE cognition scores
        per_patient_evaluation(f'{args.score}_norm', num_patients=1)

    # Print results
    print('\n****Results****')
    print('train mae', train_mae, 'validation mae', valid_mae, 'test mae', test_mae)
    # train mae 0.563 validation mae 0.447 test mae 0.478

    # Create a results dataframe to store the results
    results_df = pd.DataFrame({ 'name':name, 'seed': seed, 'gamma':gamma,'gamma_type':gamma_type, 
                               'epochs':epochs, 'batch_size':batch_size, 'cog_mtl':args.cog_mtl, 
                                'discount':args.discount, 'max_time_steps':args.trainsteps, 'w_lambda':args.w_lambda, 
                                'action_lim':args.action_limit, 'cog_init':args.cog_init, 'cog_type':args.cog_type,
                                'energy_model':args.energy_model, 'score':args.score, 'network':args.network, 'algo':args.algo, 'category':'APOE', 
