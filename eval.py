...
import os
from plot import plot_trajectories, plot_adni, plot_synthetic, plot_cognition, plot_comparison
...
class EvalPolicy:
    ...
    def simulate(self, data=None, data_type='test', scale_state=True, normalize_state=False):
        ...
        with tf.compat.v1.Session():  # Create a TensorFlow session
            ...
            for i, j in enumerate(RIDs):  # Iterate over patient IDs
                ...
                while steps < max_steps:  
                    # Get policy action (due to stochastic nature, we select the mean)
        ...
        return np.array(state_log), np.array(action_log), self.output  
    ...
    def compute(self, df, data_type='test', exp_type='adni', score='MMSE'):
        ...
        if exp_type == 'adni':
            ...
            # ********** Plotting Cognition Trajectories **********
            # plot ground truth {score}_norm' vs RL predictions (cogsc_rl)
            #plot_cognition(df_join, f'{plot_dir}/cognition_{data_type}.png', score)

             # ********** Plotting RL vs Ground Truth Trajectories **********
            plot_comparison(df_join, name="Cognition", var_rl="cogsc_rl", var_gt=score + "_norm", y_min= 5, y_max=10, filepath=f'{plot_dir}/comp_cognition_{data_type}.png')
            plot_comparison(df_join, name="Size (PFC)", var_rl="reg1_mri_rl", var_gt="mri_FRONT_norm", y_min= 0, y_max=5, filepath=f'{plot_dir}/comp_size-pfc_{data_type}.png')
            plot_comparison(df_join, name="Size (HC)", var_rl="reg2_mri_rl", var_gt="mri_HIPPO_norm", y_min= 0, y_max=5, filepath=f'{plot_dir}/comp_size-hc_{data_type}.png')
            plot_comparison(df_join, name="Size (PFC)", var_rl="reg1_D_rl", var_gt="FRONTAL_SUVR", y_min= 0, y_max=2, filepath=f'{plot_dir}/comp_amyloid-pfc_{data_type}.png')
            plot_comparison(df_join, name="Size (HC)", var_rl="reg2_D_rl", var_gt="HIPPOCAMPAL_SUVR", y_min= 0, y_max=2, filepath=f'{plot_dir}/comp_amyloid-hc_{data_type}.png')

            # ********** Plotting only Common Data Points) **********
            # Merge input data with simulation output again to generate plots on common data points (input data and RL predictions)
            ...
            # This is so that we plot the RL predictions only for those patients and those years whose real data is also available
            ...
            # Plot Ground Truth values for ADNI data
            print('Plotting Ground Truth values for ADNI data...')
            plot_adni(df_join, f'{plot_dir}/groundtruth_{data_type}.png')
            
            # Plot RL predicted values for ADNI data
            # set cognition score to be predicted cognition score (I = I_PFC + I_HC)
            df_join['cogsc'] = df_join['reg1_info_rl'] + df_join['reg2_info_rl']
            print('Plotting RL predicted values for ADNI data...')
            plot_adni(df_join, f'{plot_dir}/rl_predictions_{data_type}.png', type='_rl', score_type=score)
        ...
        # For Synthetic dataset
            ...
            try:
                # Plot Ground Truth values for Synthetic data
                print('Plotting Ground Truth values for Synthetic data...')
                plot_synthetic(df_join, f'{plot_dir}/ground_truth_{data_type}.png')

                # Plot RL predicted values for Synthetic data
                print('Plotting RL predicted values for Synthetic data...')
                plot_synthetic(df_join, f'{plot_dir}/rl_predictions_{data_type}.png', '_rl')
            except Exception as e:
                print(f"ERROR: Exception occurred while plotting plot_trajectories(): {e}")
